...
RETURNS TABLE(empleado_id INT, nombre VARCHAR(100), apellido VARCHAR(100), ocupacion VARCHAR(100)) 
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    ELSE
        ...
        RETURN QUERY SELECT 
            e.id, e.nombres, e.apellidos, oc.descripcion
        FROM 
            empleados e
        INNER JOIN 
        ...
        WHERE 
            d_e.departamento_id = vw_departamento_id;
